...
define( 'TURNSTILE_FORMS_VERSION', 'test' );
define( 'TURNSTILE_FORMS_URL', 'http://example.test/wp-content/plugins/cft/' );
define( 'TURNSTILE_FORMS_DIR', dirname( __DIR__ ) . '/' );
define( 'TURNSTILE_FORMS_BASENAME', 'cft/cft.php' );
...
require_once dirname( __DIR__ ) . '/includes/class-verifier.php';

/**
 * Reset test state between scenarios.
 *
 * @param array $options Option overrides for turnstile_forms_settings.
 * @return void
 */
function tf_reset( $options = array() ) {
	...
	$GLOBALS['__tf']['options'] = array(
		'turnstile_forms_settings' => array_merge(
			array(
				'site_key'     => 'site',
				'secret_key'   => 'secret',
				'failure_mode' => 'block',
	...
	\TurnstileForms\Verifier::reset_request_cache();
}
