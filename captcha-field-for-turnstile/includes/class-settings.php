...
class Settings {
	...
	const OPTION        = 'turnstile_forms_settings';
	const GROUP         = 'turnstile_forms_group';
	const PAGE          = 'turnstile-forms';
	const CAP           = 'manage_options';
	const IMPORT_ACTION = 'turnstile_forms_import';
	...
	public function defaults() {
		return array(
			...
			'appearance'           => 'always',
			'language'             => 'auto',
			'error_message'        => __( 'Please confirm you are not a robot.', 'captcha-field-for-turnstile' ),
			'protect_login'        => 0,
			'protect_register'     => 0,
	...
	public function add_menu() {
		add_options_page(
			__( 'Turnstile Forms', 'captcha-field-for-turnstile' ),
			__( 'Turnstile Forms', 'captcha-field-for-turnstile' ),
			self::CAP,
			self::PAGE,
	...
	public function register() {
		...
		add_settings_section(
			'turnstile_forms_keys',
			__( 'Cloudflare Turnstile keys', 'captcha-field-for-turnstile' ),
			array( $this, 'section_keys_intro' ),
			self::PAGE
		);

		add_settings_field( 'site_key', __( 'Site key', 'captcha-field-for-turnstile' ), array( $this, 'field_site_key' ), self::PAGE, 'turnstile_forms_keys' );
		add_settings_field( 'secret_key', __( 'Secret key', 'captcha-field-for-turnstile' ), array( $this, 'field_secret_key' ), self::PAGE, 'turnstile_forms_keys' );

		add_settings_section(
			'turnstile_forms_appearance',
			__( 'Widget appearance', 'captcha-field-for-turnstile' ),
			'__return_false',
			self::PAGE
		);

		add_settings_field( 'theme', __( 'Theme', 'captcha-field-for-turnstile' ), array( $this, 'field_theme' ), self::PAGE, 'turnstile_forms_appearance' );
		add_settings_field( 'size', __( 'Size', 'captcha-field-for-turnstile' ), array( $this, 'field_size' ), self::PAGE, 'turnstile_forms_appearance' );
		add_settings_field( 'appearance', __( 'Visibility', 'captcha-field-for-turnstile' ), array( $this, 'field_appearance' ), self::PAGE, 'turnstile_forms_appearance' );
		add_settings_field( 'language', __( 'Language', 'captcha-field-for-turnstile' ), array( $this, 'field_language' ), self::PAGE, 'turnstile_forms_appearance' );
		add_settings_field( 'error_message', __( 'Error message', 'captcha-field-for-turnstile' ), array( $this, 'field_error_message' ), self::PAGE, 'turnstile_forms_appearance' );

		add_settings_section(
			'turnstile_forms_native',
			__( 'WordPress forms', 'captcha-field-for-turnstile' ),
			array( $this, 'section_native_intro' ),
			self::PAGE
		);

		add_settings_field( 'protect_login', __( 'Login form', 'captcha-field-for-turnstile' ), array( $this, 'field_protect_login' ), self::PAGE, 'turnstile_forms_native' );
		add_settings_field( 'protect_register', __( 'Registration form', 'captcha-field-for-turnstile' ), array( $this, 'field_protect_register' ), self::PAGE, 'turnstile_forms_native' );
		add_settings_field( 'protect_lostpassword', __( 'Lost password form', 'captcha-field-for-turnstile' ), array( $this, 'field_protect_lostpassword' ), self::PAGE, 'turnstile_forms_native' );
		add_settings_field( 'protect_comments', __( 'Comment form', 'captcha-field-for-turnstile' ), array( $this, 'field_protect_comments' ), self::PAGE, 'turnstile_forms_native' );

		add_settings_section(
			'turnstile_forms_advanced',
			__( 'Advanced', 'captcha-field-for-turnstile' ),
			'__return_false',
			self::PAGE
		);

		add_settings_field( 'failure_mode', __( 'If Cloudflare is unreachable', 'captcha-field-for-turnstile' ), array( $this, 'field_failure_mode' ), self::PAGE, 'turnstile_forms_advanced' );
	}
	...
	public function handle_import() {
		if ( ! current_user_can( self::CAP ) ) {
			wp_die( esc_html__( 'You are not allowed to do this.', 'captcha-field-for-turnstile' ) );
		}
	...
	private function render_import_box() {
		...
		?>
		<div class="notice notice-info inline turnstile-forms-import">
			<p><strong><?php esc_html_e( 'Migrating from Simple Cloudflare Turnstile?', 'captcha-field-for-turnstile' ); ?></strong></p>
			<p><?php esc_html_e( 'We detected keys and settings from "Simple Cloudflare Turnstile". You can copy them here so you do not have to recreate your Cloudflare keys. This does not change or deactivate the other plugin.', 'captcha-field-for-turnstile' ); ?></p>
			<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
				<input type="hidden" name="action" value="<?php echo esc_attr( self::IMPORT_ACTION ); ?>" />
				<?php wp_nonce_field( self::IMPORT_ACTION ); ?>
				<?php submit_button( __( 'Import keys & settings', 'captcha-field-for-turnstile' ), 'secondary', 'submit', false ); ?>
			</form>
		</div>
		<?php
	}
		...
		printf(
			...
			wp_kses(
				sprintf(
					...
					__( 'Create a free widget and get your keys from the <a href="%s" target="_blank" rel="noopener noreferrer">Cloudflare dashboard</a>.', 'captcha-field-for-turnstile' ),
	...
	public function section_native_intro() {
		printf(
			'<p>%s</p>',
			esc_html__( 'Enable Turnstile on the built-in WordPress forms. For Elementor Pro forms, add the "Cloudflare Turnstile" field to the specific forms you want to protect.', 'captcha-field-for-turnstile' )
		);
	}
	...
	public function field_secret_key() {
		...
		printf(
			...
			esc_attr( self::OPTION ),
			esc_attr( $has_secret ? __( '••••••••  (a secret key is saved)', 'captcha-field-for-turnstile' ) : __( 'Enter your secret key', 'captcha-field-for-turnstile' ) )
		);
		...
		if ( $has_secret ) {
			printf(
				...
				esc_attr( self::OPTION ),
				esc_html__( 'Remove the saved secret key', 'captcha-field-for-turnstile' ),
				esc_html__( 'Leave the field empty to keep the saved key. The secret key is never displayed.', 'captcha-field-for-turnstile' )
			);
		} else {
			printf(
				'<p class="description">%s</p>',
				esc_html__( 'The secret key is stored server-side and never sent to the browser.', 'captcha-field-for-turnstile' )
			);
		}
	}
	...
	public function field_theme() {
		$this->render_select(
			'theme',
			array(
				'auto'  => __( 'Auto', 'captcha-field-for-turnstile' ),
				'light' => __( 'Light', 'captcha-field-for-turnstile' ),
				'dark'  => __( 'Dark', 'captcha-field-for-turnstile' ),
			)
		);
	}
	...
	public function field_size() {
		$this->render_select(
			'size',
			array(
				'normal'   => __( 'Normal', 'captcha-field-for-turnstile' ),
				'flexible' => __( 'Flexible', 'captcha-field-for-turnstile' ),
				'compact'  => __( 'Compact', 'captcha-field-for-turnstile' ),
			)
		);
	}
	...
	public function field_appearance() {
		$this->render_select(
			'appearance',
			array(
				'always'           => __( 'Always visible', 'captcha-field-for-turnstile' ),
				'interaction-only' => __( 'Only when interaction is required', 'captcha-field-for-turnstile' ),
			)
		);
	}
	...
	public function field_language() {
		...
		printf( '<p class="description">%s</p>', esc_html__( '"Auto" follows the visitor browser language.', 'captcha-field-for-turnstile' ) );
	}
	...
	public function field_protect_login() {
		$this->render_toggle( 'protect_login', __( 'Protect the WordPress login form.', 'captcha-field-for-turnstile' ) );
	}
	...
	public function field_protect_register() {
		$this->render_toggle( 'protect_register', __( 'Protect the registration form.', 'captcha-field-for-turnstile' ) );
	}
	...
	public function field_protect_lostpassword() {
		$this->render_toggle( 'protect_lostpassword', __( 'Protect the lost password form.', 'captcha-field-for-turnstile' ) );
	}
	...
	public function field_protect_comments() {
		$this->render_toggle( 'protect_comments', __( 'Protect the comment form.', 'captcha-field-for-turnstile' ) );
	}
	...
	public function field_failure_mode() {
		$this->render_select(
			'failure_mode',
			array(
				'block' => __( 'Block the submission (more secure, default)', 'captcha-field-for-turnstile' ),
				'allow' => __( 'Allow the submission (more available)', 'captcha-field-for-turnstile' ),
			)
		);
		printf(
			'<p class="description">%s</p>',
			esc_html__( 'Only applies when Cloudflare cannot be reached (network error or timeout). Invalid or missing tokens are always blocked.', 'captcha-field-for-turnstile' )
		);
	}
	...
	public function render_page() {
		...
		?>
		<div class="wrap turnstile-forms-settings">
			<h1><?php echo esc_html__( 'Turnstile for Elementor & WordPress Forms', 'captcha-field-for-turnstile' ); ?></h1>
			<?php
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only success notice after a nonce-checked redirect.
			if ( isset( $_GET['tf_imported'] ) ) {
				$imported = (int) $_GET['tf_imported']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				printf(
					'<div class="notice notice-success is-dismissible"><p>%s</p></div>',
					esc_html(
						sprintf(
							/* translators: %d: number of imported settings. */
							_n( '%d setting was imported from Simple Cloudflare Turnstile.', '%d settings were imported from Simple Cloudflare Turnstile.', $imported, 'captcha-field-for-turnstile' ),
							$imported
						)
					)
				);
	...
	public function configuration_notice() {
		...
		printf(
			...
			esc_html__( 'Captcha Field for Turnstile is active but no keys are configured yet, so no form is protected.', 'captcha-field-for-turnstile' ),
			esc_url( admin_url( 'options-general.php?page=' . self::PAGE ) ),
			esc_html__( 'Add your keys', 'captcha-field-for-turnstile' )
		);
	}
	...
	public function enqueue_admin_assets( $hook ) {
		...
		wp_enqueue_style(
			'turnstile-forms-admin',
			TURNSTILE_FORMS_URL . 'assets/css/admin.css',
			array(),
