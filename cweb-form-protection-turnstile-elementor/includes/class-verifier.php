...
class Verifier {
	...
	public function verify( $token, $remoteip = null, $expected_action = null ) {
		...
		if ( null !== $expected_action && apply_filters( 'cwebts_verify_action', false ) ) {
		...
		if ( apply_filters( 'cwebts_verify_hostname', false ) ) {
			$default_host = wp_parse_url( home_url(), PHP_URL_HOST );
			$allowlist    = (array) apply_filters( 'cwebts_hostname_allowlist', array( $default_host ) );
	...
	private function siteverify( $token, $remoteip ) {
		...
		$timeout = (int) apply_filters( 'cwebts_timeout', 5 );
	...
	private function resolve_remote_ip( $override ) {
		 ...
		 */
		$ip = apply_filters( 'cwebts_remoteip', $override );
