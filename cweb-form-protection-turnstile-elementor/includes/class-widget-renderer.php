...
class Widget_Renderer {
	...
	const API_HANDLE = 'cwebts-api';
	const JS_HANDLE  = 'cwebts';
	...
	private function register_scripts() {
		...
		wp_register_script(
			self::JS_HANDLE,
			CWEBTS_URL . 'assets/js/turnstile.js',
			array(),
			CWEBTS_VERSION,
			true
		);
		...
		// Version it with the plugin version (Cloudflare ignores the extra arg).
		wp_register_script( self::API_HANDLE, $api_url, array( self::JS_HANDLE ), CWEBTS_VERSION, true );
	}
	...
	public function get_html( $overrides = array() ) {
		...
		$attributes = array(
			'class'           => trim( 'cf-turnstile cwebts-widget ' . $args['class'] ),
